...
↺ 𝘽𝙖𝙣𝙠 -» <code>{bank}</code>
↺ 𝘾𝙤𝙪𝙣𝙩𝙧𝙮 -» <code>{country_name} {country_flag} {ccurrencies}</code>
━━━━━━━━━━━━━━
𝘽𝙤𝙩 𝘽𝙮 - @nophq"""
token = input('7356315385:AAEL9tESykYYQoB1NKsZ9dcdT3PCTTyrcDE: ')

#Bot by @Beginnercarder
...
def start(message):
    if message.from_user.id:
        ...
        bot.reply_to(message,f"Welcome to B3 HQ Checker!/nDrop your CCs Combo file below 👇🏻/n🚀 Maximum CCs: 50 for now!\n\n- 𝘽𝙤𝙩 𝘽𝙮 - @nophq",parse_mode="markdown")
...
def send_file(message):
	...
	af = types.InlineKeyboardButton('💎 Owner', url='https://t.me/nophq')
	key.add(af)
...
#Bot by BlackHeads
	...
	for g in open(f"{message.document.file_name}","r").read().splitlines():
		...
		ccnn = types.InlineKeyboardButton(f" 🌿 CCNs: {ccn} ", callback_data="cvv")
		cvvv = types.InlineKeyboardButton(f" 💚 CVVs: {cvv} ", callback_data="cvv")
		...
		nch = types.InlineKeyboardButton(f" 💳 CCs: {nc} ", callback_data="chk")
		own = types.InlineKeyboardButton(f"💎 Owner", url="https://t.me/nophq")
		key.add(ccli,badd,nch,ap,ccnn, cvvv,own )
		bot.edit_message_text(chat_id=message.chat.id, message_id=idmss.message_id,text="Checker: Running ✨️\n- 𝘽𝙤𝙩 𝘽𝙮 - @Beginnercarder", reply_markup=key)
		time.sleep(37)
